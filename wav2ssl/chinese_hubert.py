...
class CNHubert(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = HubertModel.from_pretrained("TencentGameMate/chinese-hubert-large")
        self.feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("TencentGameMate/chinese-hubert-large")
    def forward(self, x):
        input_values = self.feature_extractor(x, return_tensors="pt").input_values.to(x.device)
